...
# Isso abrirá uma caixa de texto no Jupyter Notebook!
porta_digitada = input("🎯 Digite a porta desejada (ex: 5202, 34098) ou tecle ENTER para ver todas: ").strip()

df = pd.DataFrame(historico_dados)

if not df.empty:
    # 🌟 CONVERSÃO: Transformando SRTT (µs) em RTT (ms)
    df['rtt_ms'] = df['srtt'] / 1000.0

    if porta_digitada.isdigit():
        porta = int(porta_digitada)
        # Filtra a tabela onde a porta de origem OU a porta de destino seja igual à digitada
        df = df[(df['porta_origem'] == porta) | (df['porta_destino'] == porta)]
    ...
    if not df.empty:
        ...
        titulo = f"Análise TCP (Visão Limpa) - Porta {porta_digitada if porta_digitada else 'Todas'}"
        ...
        for idx, ((src, dst), grupo) in enumerate(grupos):
            ...
            # Exigimos apenas que a janela seja de pelo menos 10 pacotes
            if grupo['cwnd'].max() >= 10: 
        ...
        if linhas_validas > 0:
            ...
            nome_arquivo = f"Grafico_TCP_Interativo_{datetime.now().strftime('%H%M%S')}.png"
            plt.savefig(nome_arquivo, dpi=300, bbox_inches='tight')
            ...
            df_limpo = df.groupby(['porta_origem', 'porta_destino']).filter(lambda x: x['cwnd'].max() >= 10)
            print(df_limpo[colunas_stats].describe().round(2))

        else:
            print(f"⚠️ A captura não possui conexões ativas (Cwnd >= 10) para a porta '{porta_digitada}'.")
    else:
        print(f"⚠️ Nenhum dado encontrado para a porta '{porta_digitada}'. Tente teclar ENTER para ver todas.")
else:
    print(f"❌ Nenhum tráfego detectado.")
